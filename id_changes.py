...
def get_updated_elements(master_bom_df, new_bom_df, compare_columns=[None], deleted_idx=[None],
    ...
    master_bom_df_subset = master_bom_df.loc[~deleted_idx, compare_columns].reset_index().set_index(node_id).drop(columns=drop)
    new_bom_df_subset = new_bom_df.loc[~added_idx, compare_columns].reset_index().set_index(node_id).drop(columns=drop)
